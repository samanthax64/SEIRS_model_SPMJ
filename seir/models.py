...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def system_dfes(t, variables, beta, sigma, gamma, xi, mu_I, mu_0, nu,
        ...
        N   = S + E + I + D_E + D_I + R - deltaN

        dS  = - (beta*S*I)/(N+deltaN) - q*(beta_D*S*D_I)/(N+deltaN) + xi*R + nu*(N+deltaN) - mu_0*S

        dE  = (beta*S*I)/(N+deltaN) + q*(beta_D*S*D_I)/(N+deltaN) - sigma*E - theta_E*psi_E*E - mu_0*E
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_epoch(self, runtime, dt=0.1, pop_list_epoch=[], meses_epoch=[], dfParam_epoch=[]):
    # def run_epoch(self, runtime, dt=1.0):
        #Converter parametros (df) em lista
        tresus_list = dfParam_epoch['taxa_resusceptibilidade'].to_list()
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Ajuste de parametros ao longo do tempo:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        position=0
        init_cond       = [self.numS[-1], self.numE[-1], self.numI[-1], self.numD_E[-1], self.numD_I[-1], self.numR[-1], self.numF[-1]]
        if self.t >0 and len(meses_epoch) >0:
            #ipdb.set_trace()
            try:
                position = numpy.where(numpy.array(meses_epoch)==numpy.round(self.t))[0][0]
            except:
                ipdb.set_trace()
        if position == 0:
           init_cond[2]+= dfParam_epoch.infect_add/2
           init_cond[1]+= dfParam_epoch.infect_add
        if position>0:
            # Samantha - abordagem aumentando a população exposta inicial. Outra possibilidade seria definir a população exposta.
            #init_cond[0] = self.N*(pop_list_epoch[position] - pop_list_epoch[position - 1]) + init_cond[0] #aumentando a população exposta
            #if position == 22:
                #init_cond[1] =0
                #init_cond[2] = init_cond[0]/20
                #init_cond[5] =0
                #init_cond[6] =0
                #ipdb.set_trace()
            
            self.N = pop_list_epoch[position]* dfParam_epoch.pop
            #init_cond = [init_cond[i]*dfParam_epoch.pop for i in range(0,len(init_cond))] 
            #Samantha - Calcular pop_list_epoch usando position para distanciamento
            #           E para vacinação deslocar o position analisando a data. 
            #           Lógica de cálculo dentro de Samantha_estimator, olhar variável pop_list, deslocando count de acordo com os dias de antecipação. 
            
            try:
                prop = min(1,numpy.abs(self.N - self.Nant)/self.Nant)
            except:
                prop = 1
            ...
            init_cond[0] = self.N - numpy.sum(init_cond[1:])
            #if init_cond[0] <0:
                #print(init_cond[0])
            #if init_cond[0]<0:
                #init_cond[6] *= prop
                ...
                #ipdb.set_trace()
            self.Nant = self.N
            self.xi = tresus_list[position] #Ajustando taxa de resusceptibilidade
        #init_cond[2] = min(init_cond[2],init_cond[0])
        #init_cond[1] = min(init_cond[1],init_cond[0])
        #init_cond[6] =0
        #if position == 0:
            #ipdb.set_trace()
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        #ipdb.set_trace()
        if dfParam_epoch.parametrosCode['AntVac']>0:
            for i in range(0,7):
                for j in range(0,len(solution['y'][i])):
                    solution['y'][i][j] = max(0,solution['y'][i][j])
        ...
        ti_max = self.tseries[-1]
        for ti in t_eval:
            if ti> ti_max:
                self.tseries    = numpy.append(self.tseries, ti)
                self.numS       = numpy.append(self.numS, self.numS[-1])
                self.numE       = numpy.append(self.numE, self.numE[-1])
                self.numI       = numpy.append(self.numI, self.numI[-1])
                self.numD_E     = numpy.append(self.numD_E, self.numD_E[-1])
                self.numD_I     = numpy.append(self.numD_I, self.numD_I[-1])
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run(self, T, dt=0.1, checkpoints=None, pop_list_run=[], meses_run=[],dfParam_run =[], verbose=False, printProgress_run=True):
    # def run(self, T, dt=1.0, checkpoints=None, verbose=False):
        ...
        #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if(not checkpoints):
            self.run_epoch(runtime=self.tmax, dt=dt, pop_list_epoch=pop_list_run, meses_epoch=meses_run, dfParam_epoch = dfParam_run)

            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

            if printProgress_run:
                print("t = %.2f" % self.t)
            if(verbose):
            ...
            for checkpointIdx, checkpointTime in enumerate(checkpoints['t']):
                #if checkpointIdx >= 22: ipdb.set_trace()
                # Run the sim until the next checkpoint time:
                self.run_epoch(runtime=checkpointTime-self.t, dt=dt, pop_list_epoch=pop_list_run, meses_epoch=meses_run, dfParam_epoch = dfParam_run)
            ...
            if(self.t < self.tmax):
                self.run_epoch(runtime=self.tmax-self.t, dt=dt, pop_list_epoch=pop_list_run, meses_epoch=meses_run, dfParam_epoch = dfParam_run)
