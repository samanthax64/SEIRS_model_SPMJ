...
def modelo_SEIRS_plus(local=local, tdias=T_dias, ndias=Ndias,
                      ...
                      sl_pop=slider_pop, pop_list = [],
                      dfParam = [],
                      imprime=False, pop_real=False,
                      printProgress_seirbr=True,
                      meses_mud_beta = [], beta_list = [], theta_E_list = [], theta_I_list = []
                      ):
    ...
    beta_ref=beta_depois
    #ipdb.set_trace()
    '''
    checkpoints = {'t': [1, 169, 170,180],
               # Taxa de infecção do covid reduz e retoma
               # O distanciamento produz uma redução de transmissão até 74%
               'beta':    [beta_ref, beta_ref, beta_ref/10, beta_ref],
               # Taxa de testes e detecção do covid
               # no período de quarentena
               'theta_E': [Testes, Testes, Testes,Testes],
               'theta_I': [Testes, Testes, Testes,Testes]
               # 'theta_E': [0.02, 0.02],
               # 'theta_I': [0.02, 0.02]
              }
    '''
    checkpoints = {'t': meses_mud_beta,               
               'beta': beta_list,
               'theta_E': theta_E_list,
               'theta_I': theta_I_list
              
              }
    
    
    # model.run(T=NDIAS)
    model.run(T=NDIAS, checkpoints=checkpoints, pop_list_run=pop_list, meses_run = meses_mud_beta, dfParam_run = dfParam, printProgress_run = printProgress_seirbr)
