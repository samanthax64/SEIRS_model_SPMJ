...
# CITY_DATA_URL = './seir/utils/cases-brazil-cities-time.csv'
# https://github.com/wcota/covid19br/raw/master/cases-brazil-cities-time.csv.gz
# STATE_DATA_URL = "https://github.com/wcota/covid19br/raw/master/cases-brazil-cities-time.csv.gz"
# file = pd.read_csv(URL + "",
#                    compression='gzip', encoding='utf-8')
STATE_DATA_URL = './seir/utils/cases-brazil-cities-time.csv'
...
def load_data():
    ...
    state_df = (
                ...
                .drop(['ibgeID', 'country', 'city',
                       ...
                       'name_RegiaoDeSaude', 'last_info_date',
                       '_source', 'epi_week',
                       'cod_RegiaoDeSaude'], axis=1)
                .replace('TOTAL', 'Brazil')
    )

    state_df['confirmed_new'] = state_df.confirmed_new.abs()
    state_df['deaths_new'] = state_df.deaths_new.abs()

    state_df = state_df.assign(state=lambda x: x['state']).groupby(['state', 'date']).sum()

    # print('state_df: ', state_df.filter(like='SC', axis=0))
    # state_df_sc = state_df.filter(like='SC', axis=0)
    # state_df_sc.to_csv('temp.csv')
...
def smooth_new_cases(new_cases):
    ...
    smoothed_cases = new_cases.rolling(21,
        win_type='gaussian',
        min_periods=1,
...
def calculate_posteriors(sr, sigma=0.15):
    ...
    for previous_day, current_day in zip(sr.index[:-1], sr.index[1:]):
        ...
        # denominator = np.nan_to_num(np.sum(numerator))
...
def highest_density_interval(pmf, p=.9):
    ...
    if(isinstance(pmf, pd.DataFrame)):
        # print("Retorno: ", pd.DataFrame([highest_density_interval(pmf[col], p=p) for col in pmf],
        #                     index=pmf.columns))
        # print("PMF: ",pmf)
        # print("PMF sum: ",pmf.sum(axis = 0, skipna = True))
        # soma = pmf.sum(axis = 0, skipna = True)
        # print('soma[0]: ', soma[0])
        # print('soma[soma == 0.0]: ', soma[soma == 0.0])
        # print("PMF sum < 1: ",pmf.sum(axis = 0, skipna = True))
...
def plot_rt(result, ax, state_name):
    ...
    ax.margins(0)
    ax.set_ylim(0.0, 5.0)
    ax.set_xlim(pd.Timestamp('2020-03-01'), result.index.
                get_level_values('date')[-1] +
                pd.Timedelta(days=1))


# def plot_standings(mr, figsize=None, title='Most Recent $R_t$ by State'):
def plot_standings(mr, figsize=None, title='$R_t$ mais recente por estado'):
    ...
    fig.savefig('./images/Rt-state-comparison.png')
...
def run_full_model(cases, sigma=OPTIMAL_SIGMA):
    ...
    posteriors, log_likelihood = calculate_posteriors(
        np.clip(smoothed, 0, None), sigma=sigma)

    # calculating HDI
    result = highest_density_interval(posteriors, p=.9)

    # adding new case and smoothed new case data
    result['New_Cases'] = new
    result['Smoothed_Cases'] = smoothed
